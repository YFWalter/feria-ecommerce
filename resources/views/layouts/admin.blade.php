...
<div class="flex h-screen overflow-hidden" x-data="{ sidebarOpen: false }">
    ...
    <aside class="fixed md:static inset-y-0 left-0 z-40 w-64 bg-gray-900 text-gray-100 flex flex-col flex-shrink-0 transform transition-transform duration-200 md:translate-x-0"
        ...
        <nav class="flex-1 px-4 py-4 space-y-1">
            ...
            {{-- Configuración (desplegable, separado del resto) --}}
            <div class="pt-2 mt-2 border-t border-gray-700"
                 x-data="{ open: {{ request()->routeIs('admin.configuracion.*') ? 'true' : 'false' }} }">
                <button type="button" @click="open = !open"
                        class="w-full flex items-center justify-between gap-3 px-3 py-2 rounded-lg text-sm transition-colors {{ request()->routeIs('admin.configuracion.*') ? 'bg-primary-500 text-gray-900 font-semibold' : 'text-gray-300 hover:bg-gray-700' }}">
