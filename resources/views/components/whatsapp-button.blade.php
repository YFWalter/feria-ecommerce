@php
    // Dejamos solo los dígitos del número (formato que espera wa.me).
    $waNumber = preg_replace('/\D+/', '', (string) setting('whatsapp_number'));
@endphp

@if($waNumber)
    <a href="https://wa.me/{{ $waNumber }}?text={{ rawurlencode(setting('whatsapp_message')) }}"
       target="_blank" rel="noopener noreferrer"
       class="fixed bottom-5 right-5 z-50 flex items-center justify-center w-14 h-14 rounded-full bg-green-500 hover:bg-green-600 text-white shadow-lg hover:shadow-xl transition-all"
